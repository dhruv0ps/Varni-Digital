...
const FourSwitch: React.FC<{ switchId: string, CloseIconComponent?: () => JSX.Element, }> = ({ switchId, CloseIconComponent }) => {
    // Generate unique IDs based on the passed switchId and index
    const fourSwitch = Array.from({ length: 2 }, (_, index) => `${switchId}-fourSwitch-${index}`);
